...
install_x_ui() {
    ...
    tag_version=$(curl -Ls "https://api.github.com/repos/MetaQuest/3x-ui-Apline/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    wget --no-check-certificate -O x-ui-linux-alpine.tar.gz "https://github.com/MetaQuest/3x-ui-Apline/releases/download/${tag_version}/x-ui-linux-amd64.tar.gz"
    if [[ $? -ne 0 ]]; then
    ...
    wget --no-check-certificate -O /etc/init.d/x-ui https://raw.githubusercontent.com/MetaQuest/3x-ui-Apline/refs/heads/main/x-ui.rc
